...
type AlbumUseCase interface {
	CreateAlbum(title string, releaseDate time.Time, coverURL string) (*models.Album, error)
	...
	UpdateAlbumInfo(albumID uuid.UUID, title, coverURL string, releaseDate time.Time) error
}
